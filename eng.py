...
TELEGRAM_BOT_TOKEN = '8036559880:AAG6IFYFlPCBeQc18y17_ReVB4X3I3Nzn_E'
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*🎉 Welcome to the roxz Ultimate UDP Flooder! 🎉*\n\n"
...
async def roxz(update: Update, context: CallbackContext):
    ...
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Incorrect usage! Use: /roxz <add|rem> <user_id> <coins>*", parse_mode='Markdown')
        return
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user["coins"] < COINS_REQUIRED_PER_ATTACK:
        await context.bot.send_message(
            chat_id=chat_id,
            text="*💰 Insufficient coins! Please contact the admin to acquire more coins. DM: @Roxz_gaming*",
            parse_mode='Markdown'
        )
        return
...
async def help(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    message = (
        "*🛠️ roxz VIP DDOS Bot Help Menu 🛠️*\n\n"
        ...
        "- *If you encounter any issues, contact the admin: @Roxz_gaming*\n\n"
